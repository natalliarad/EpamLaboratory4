...
public class MyArrayList<E> implements List<E> {
  ...
  /**
   * Check a capacity of the MyArrayList. <p> If the capacity of the MyArrayList is not enough, then
  ...
  private void checkArrayRange(int index) {
    if (index > size || index < 0) {
      throw new IndexOutOfBoundsException(
          "Index " + index + "isn\'t allowed. Size of the MyArrayList: " + size);
    }
  }

  /**
   * To check whether the indexes go beyond the boundaries of the MyListArray, as well as the
   * correctness of the initial and final indices.
   *
  ...
  private void sublistCheckArrayRange(int fromIndex, int toIndex, int size) {
    ...
    if (toIndex > size) {
      throw new IndexOutOfBoundsException(
          "The toIndex (" + toIndex + ") is more than the size of MyArraylist ("
              + size + ").");
    }
  ...
  public static void main(String[] args) {
    ...
    list1.clear();
    System.out
        .println("Print the size of the MyListArray after deleting all elements - " + list1.size());
  }
}
