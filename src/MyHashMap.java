...
public class MyHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V> {
    ...
    public MyHashMap(int initialCapacity, float loadFactor) {
        if (initialCapacity < 0)
            throw new IllegalArgumentException("Incorrect initial capacity of the MyHashMap<K, V>: " + initialCapacity);
        if (initialCapacity > MAXIMUM_CAPACITY)
            initialCapacity = MAXIMUM_CAPACITY;
        if (loadFactor <= 0 || Float.isNaN(loadFactor))
            throw new IllegalArgumentException("Incorrect load factor of the MyHashMap<K, V>: " + loadFactor);
        int capacity = 1;
    ...
    public V get(Object key) {
        ...
        for (Entry<K, V> element = elementData[indexFor(hash, elementData.length)]; element != null; element = element.next) {
            K k;
    ...
    private Entry<K, V> getEntryKey(K key) {
        ...
        for (Entry<K, V> element = elementData[indexFor(hash, elementData.length)]; element != null; element = element.next) {
            K k;
            if (element.hash == hash && (element.key == key || (key != null && key.equals(element.key))))
                return element;
        }
        return null;
    }
